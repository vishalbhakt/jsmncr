...
import api, { assignmentsAPI, subjectsAPI } from "@/lib/api";
import { 
  Plus, 
  Trash2, 
  Calendar, 
...
export default function TeacherAssignments() {
  ...
  const fetchData = async () => {
    ...
    try {
      ...
      addToast("Failed to sync assignment drive", "error");
    } finally {
      setLoading(false);
    }
  };
    ...
    try {
      ...
      if (res.data.success) {
        ...
        const errorMsg = typeof res.data.error === 'object' 
          ...
          : res.data.error || "Failed to publish assignment";
        ...
        <div className="flex flex-col md:flex-row md:items-center justify-between gap-6">
          <div>
            ...
            <p className="text-slate-500 font-medium mt-1">
              Publish and evaluate student tasks.
            </p>
          </div>
          <button 
            ...
            >
              ...
              <form onSubmit={handleSubmit} className="grid grid-cols-1 md:grid-cols-2 gap-8 relative z-10">
                ...
                <div className="col-span-2 space-y-2">
                  ...
                  <textarea className="w-full bg-slate-50 border-2 border-slate-100 rounded-2xl px-6 py-4 font-bold focus:border-gold outline-none min-h-[120px] transition-all"
                ...
                <div className="space-y-2">
                  ...
                  <input type="datetime-local" required className="w-full bg-slate-50 border-2 border-slate-100 rounded-2xl px-6 py-4 font-bold focus:border-gold outline-none"
                ...
                <button 
                  type="submit" 
                  disabled={saving}
                  className="col-span-2 btn-primary py-5 rounded-2xl flex items-center justify-center gap-3 shadow-xl shadow-navy/20 disabled:opacity-70"
                >
                  {saving ? (
          ...
          <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-8">
            {items.map((item) => (
              <div key={item.id} className="bg-white p-8 rounded-[2.5rem] border border-navy/5 shadow-sm hover:border-gold/30 transition-all flex flex-col group relative">
                <div className="absolute top-4 right-4 p-2 text-slate-200 group-hover:text-navy transition-all cursor-default opacity-0 group-hover:opacity-100">
                   <MoreVertical className="w-5 h-5" />
                </div>
                
                <div className="flex items-center gap-2 mb-6">
                   <div className="px-3 py-1 bg-navy/5 text-navy text-[10px] font-black uppercase tracking-widest rounded-lg">{item.subject_name}</div>
                </div>
                ...
                <div className="space-y-4">
                   <button 
                    ...
                    className="w-full bg-navy text-white py-3.5 rounded-xl text-[10px] font-black uppercase tracking-widest hover:bg-gold transition-all shadow-lg active:scale-95"
                   >
                      Review Submissions
                   </button>
            ...
            {!items.length && !isAdding && (
              <div className="col-span-full py-40 text-center bg-white rounded-[3rem] border-2 border-dashed border-navy/5 flex flex-col items-center">
                 ...
                 <p className="text-slate-400 font-bold uppercase tracking-widest text-xs">No active assignments in your drive.</p>
              </div>
            )}
          </div>
        )}
