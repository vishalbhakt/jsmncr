    ...
    <DashboardLayout>
      <div className="space-y-10 pb-20">
        ...
        <div className="grid grid-cols-1 lg:grid-cols-3 gap-8">
           <div className="lg:col-span-1 bg-white p-10 rounded-[3rem] border border-navy/5 shadow-sm flex flex-col justify-between h-72 group hover:border-gold/30 transition-all relative overflow-hidden">
              <div className="space-y-1 relative z-10">
                 <div className="text-[10px] font-black uppercase text-slate-400 tracking-widest">Outstanding Dues</div>
                 <div className="text-5xl font-black text-navy group-hover:text-gold transition-colors">₹{totalPending.toLocaleString()}</div>
              </div>
           ...
           <div className="lg:col-span-2 bg-navy rounded-[3rem] p-10 text-white shadow-2xl flex flex-col md:flex-row items-center gap-10 relative overflow-hidden">
              <div className="space-y-6 relative z-10">
                 <h3 className="text-2xl font-black text-gold uppercase tracking-wider">Fee Policy</h3>
              ...
              <div className="shrink-0 relative z-10">
                 <div className="w-32 h-32 bg-white/5 rounded-full flex items-center justify-center border border-white/10">
                    <Receipt className="w-12 h-12 text-gold" />
                 </div>
              </div>
              {/* Decor */}
              <div className="absolute top-0 right-0 w-64 h-64 bg-gold/5 rounded-full blur-3xl translate-x-1/2 -translate-y-1/2"></div>
           </div>
        </div>

        <div className="bg-white rounded-[2.5rem] border border-navy/5 shadow-sm overflow-hidden">
           <div className="p-8 border-b border-navy/5 font-black text-navy flex items-center gap-3 uppercase tracking-widest text-xs">
              <Receipt className="w-4 h-4 text-gold" /> Ledger History
           </div>
                ...
                <tbody className="divide-y divide-navy/5">
                   {data.map((p, i) => (
                     <tr key={i} className="hover:bg-slate-50/50 transition-all">
                        <td className="px-8 py-5 font-bold text-navy">{new Date(p.due_date).toLocaleDateString('en-IN', { day: 'numeric', month: 'short', year: 'numeric' })}</td>
                        <td className="px-6 py-5 text-slate-500 font-medium">{p.description || "Monthly Tuition"}</td>
                        <td className="px-6 py-5 font-black text-navy text-lg">₹{parseFloat(p.amount).toLocaleString()}</td>
                        ...
                        <td className="px-8 py-5 text-right">
                           {p.status === 'Paid' ? (
                             <button className="p-2 bg-navy/5 text-navy rounded-xl hover:bg-navy hover:text-white transition-all"><Download className="w-4 h-4" /></button>
                           ) : (
