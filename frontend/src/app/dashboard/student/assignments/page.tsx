...
import api, { assignmentsAPI } from "@/lib/api";
import { 
  Clock, 
  Download, 
  Send,
...
export default function StudentAssignments() {
  ...
  const handleAssignmentSubmit = async (assignmentId: number) => {
    ...
    try {
      const res = await api.post("/assignment-submissions/", fd, { headers: { 'Content-Type': 'multipart/form-data' } });
      if (res.data.success) {
        ...
        addToast(res.data.error || "Submission failed.", "error");
      }
    } catch (err: any) {
      const msg = err.response?.data?.error || "Submission failed. You might have already submitted.";
      addToast(msg, "error");
    } finally {
      setSubmitting(false);
    }
      ...
      <div className="space-y-10 pb-20">
        ...
        {loading ? (
          <div className="grid grid-cols-1 lg:grid-cols-2 gap-6">
            {[1,2,3,4].map(i => <div key={i} className="h-48 bg-white rounded-[2.5rem] animate-pulse border border-navy/5"></div>)}
          </div>
        ) : (
          <div className="grid grid-cols-1 lg:grid-cols-2 gap-6">
            {data.map((item) => (
              <div key={item.id} className="bg-white p-8 rounded-[2.5rem] border border-navy/5 shadow-sm flex flex-col md:flex-row md:items-center justify-between gap-8 group hover:border-gold/30 transition-all relative overflow-hidden">
                <div className="space-y-4 max-w-xl relative z-10">
                  <div className="flex items-center gap-3">
                    <span className="px-3 py-1 bg-navy/5 text-navy text-[10px] font-black uppercase tracking-widest rounded-lg">{item.subject_name}</span>
                    <span className={`text-[10px] font-black uppercase tracking-widest flex items-center gap-1 ${item.is_submitted ? "text-emerald-500" : "text-rose-500"}`}>
                      {item.is_submitted ? <CheckCircle2 className="w-3.5 h-3.5" /> : <Clock className="w-3.5 h-3.5" />}
                      {item.is_submitted ? "Submitted" : `Due: ${new Date(item.due_date).toLocaleDateString()}`}
                    </span>
                  </div>
                ...
                <div className="shrink-0 flex flex-col gap-3 relative z-10">
                  {item.file && (
                    <a href={item.file} target="_blank" className="flex items-center justify-center gap-2 w-full bg-navy text-white px-6 py-3 rounded-xl text-[10px] font-black uppercase tracking-widest hover:bg-navy-light transition-all shadow-lg shadow-navy/10">
                  ...
                  >
                    {item.is_submitted ? "Resubmit Work" : "Hand In Now"}
                  </button>
                </div>
        ...
        <AnimatePresence>
          {isSubmitting && (
            <div className="fixed inset-0 z-[100] bg-navy/60 backdrop-blur-md flex items-center justify-center p-6">
              <motion.div initial={{ scale: 0.9, opacity: 0 }} animate={{ scale: 1, opacity: 1 }} exit={{ scale: 0.9, opacity: 0 }} className="bg-white rounded-[3rem] p-12 max-w-md w-full space-y-8 shadow-2xl relative overflow-hidden">
                 ...
                 <div className="text-center relative z-10">
                    <h3 className="text-3xl font-black text-navy">Submit Solution</h3>
                    <p className="text-slate-400 font-medium mt-1">Upload your completed task for evaluation.</p>
                 </div>
                 <div className="space-y-6 relative z-10">
                    <div className="border-2 border-dashed border-slate-200 rounded-3xl p-10 text-center hover:border-gold/50 transition-all group bg-slate-50 cursor-pointer relative overflow-hidden">
                       <input 
                         type="file" 
